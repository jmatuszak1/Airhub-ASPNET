...
namespace Airhub.Controllers
{
    public class PassengerController : Controller
    {
        ...
        {
            return View(await _context.Passengers
                .Include( x => x.Flight)
                .ThenInclude( f => f.ArrivalAirport)
                .Include(x => x.Flight)
                .ThenInclude(f => f.DepartureAirport).ToListAsync());
        }
        //GET CUSTOMER'S FLIGHTS
        public async Task<IActionResult> Index(int id)
        {

            var passengers = await _context.Passengers.Where(j => j.UserId == id)
                .Include(x => x.Flight)
                .ThenInclude(f => f.ArrivalAirport)
                .Include(x => x.Flight)
                .ThenInclude(f => f.DepartureAirport).ToListAsync();
        ...
        public async Task<IActionResult> MyFlightsHistory(int id)
        {
            var passengers =  await _context.Passengers.Where(j => j.UserId == id)
                .Include(x => x.Flight)
                .ThenInclude(f => f.ArrivalAirport)
